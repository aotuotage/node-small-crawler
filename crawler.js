...
class Crawler {
  constructor (options) {
    const { dir = './imgs', proxyUrl = originUrl, questionId = '340990864', offset = 0, limit = 10, timeout = 10000 } = options
  ...
  downloadAllImg (cb) {
    ...
    this.imgs.forEach((imgUrl) => {
      let imgClear = imgUrl.replace(/\?(.*)/, '')
      let fileName = path.basename(imgClear)
